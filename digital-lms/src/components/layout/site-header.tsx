...
import { getAllowSignup, getEnableJobs } from "@/lib/public-access";
import { SiteHeaderClient } from "@/components/layout/site-header-client";

export async function SiteHeader() {
  const session = await getSession();
  const staff = isStaff(session?.roles);
  const [allowSignup, enableJobs] = await Promise.all([getAllowSignup(), getEnableJobs()]);
  ...
  return (
    <footer className="mt-auto border-t border-stone-200 bg-white">
      <div className="mx-auto flex max-w-6xl flex-col gap-2 px-4 py-8 text-sm text-stone-500 md:flex-row md:items-center md:justify-between">
        ...
        <p>Courses · Live batches · Certificates</p>
      </div>
    </footer>
  );
}
