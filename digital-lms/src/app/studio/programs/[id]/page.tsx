...
export default function EditProgramPage() {
  ...
  useEffect(() => {
    ...
    void fetch(`/api/programs/${id}`)
      .then((r) => r.json())
      .then((data) => {
        if (data.title) {
          setTitle(data.title);
          setDescription(data.description || "");
          setCourseIds((data.courseIds || []).map(String));
          setEnforceOrder(!!data.enforceOrder);
  ...
  return (
    <div className="mx-auto max-w-xl space-y-6">
      <h1 className="font-serif text-3xl text-blue-950">Edit program</h1>
      <Card>
        <CardHeader>
