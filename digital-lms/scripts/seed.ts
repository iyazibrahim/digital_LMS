...
async function main() {
  ...
  const adminEmail = process.env.SEED_ADMIN_EMAIL || "admin@digitalpenang.my";
  ...
  let instructor = await User.findOne({ email: "instructor@digitalpenang.my" });
  if (!instructor) {
    instructor = await User.create({
      email: "instructor@digitalpenang.my",
  ...
  let student = await User.findOne({ email: "student@digitalpenang.my" });
  if (!student) {
    student = await User.create({
      email: "student@digitalpenang.my",
      name: "Ravi Student",
          ...
          lessons: [
            {
              ...
              isPreview: true,
              contentHtml:
                "<p>Welcome! This course guides you step by step through essential digital skills.</p><p>Complete each lesson, take the quiz, and submit your reflection to earn a certificate.</p>",
              durationMinutes: 5,
            },
            {
  ...
  let batch = await Batch.findOne({ slug: "april-cohort" });
  if (!batch) {
  ...
  console.log("Instructor: instructor@digitalpenang.my / instructor123");
  console.log("Student: student@digitalpenang.my / student123");
  process.exit(0);
}
